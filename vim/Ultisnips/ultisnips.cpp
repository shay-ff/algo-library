...
$0 endsnippet
    ...
    snippet vect "vector" vector<${1 : int}>
${2 : arr};
$0 endsnippet

    snippet all "all" ${1 : arr}
        .begin(),
    $1.end() $0 endsnippet

    snippet readvec "read vector" vector<${1 : int}> ${2 : arr}(${3 : n});
for ($1 &val : $2) {
  cin >> val;
}
$0 endsnippet

    snippet sort "read vector" sort(${1 : arr}.begin(), $1.end());
$0 endsnippet

    snippet pb "push_back" push_back($1);
$0 endsnippet

    snippet graph "read graph" const int N = ;
...
endsnippet
    ...
    snippet tree "read tree" const int N = ;
...
endsnippet
    ...
    snippet rootedtree "read rooted tree" const int N = ;
...
endsnippet
    ...
    snippet 0rootedtree "read rooted in first vertex tree" const int N = ;
...
endsnippet
    ...
    snippet gcd "gcd" template <typename T>
    T gcd(T a, T b) {
  while (a) {
    b %= a;
    swap(a, b);
...
endsnippet
    ...
    snippet binpow "binpow" template <typename T>
    T binpow(T a, T b) {
  T ans = 1;
  while (b) {
    if (b & 1) {
...
endsnippet
    ...
    snippet inv "any mod inverse" ll
    inv(ll a, ll m) {
  if (a == 1) {
    return 1;
  }
...
endsnippet
  ...
  vector<cmpl> roots = {
      {0, 0},
      {1,
       0}}; // complex roots of 1 (with bases from 1 to base), 1-based indexing
...
endsnippet
    ...
    snippet sufarr "suffix array" const char C =
        'a' - 1;          // before first letter // change
...
endsnippet
    ...
    snippet aho "aho-corasik" struct aho {
  ...
  aho() : g(), gr(), str(), root(0), sz(0), ending(), link() {}
  ...
  endsnippet
      ...
      snippet cht "convex hull trick" typedef long long integer;

  struct Line {
    integer k, b;
    Line() : k(0), b(0) {}
    Line(integer k, integer b) : k(k), b(b) {}
  ...
  endsnippet
      ...
      snippet segtree "segment tree" struct SegmentTree {
    // TO CHANGE
    ...
    void update(int ql, int qr,
                const T &newval) { // update [ql, qr) with newval
    ...
    int find_first(const function<bool(const Node &)>
                       &predicate) { // find first index that satisfies
                                     // predicate or -1 if none
      return find_first(0, 0, n, 0, n, predicate);
    }
  };
  endsnippet

      snippet centroid "centroid decomposition" const int MAXN = ;
  ...
  endsnippet
      ...
      snippet sparse "sparse table" template <typename T>
      struct SparseTable {
    ...
    SparseTable(const vector<T> &arr,
      ...
      for (int lg = 1; lg < logn; lg++) {
        for (int i = 0; i + (1 << lg) <= n; i++) {
          sparse[lg][i] = accum_func(sparse[lg - 1][i],
                                     sparse[lg - 1][i + (1 << (lg - 1))]);
        }
      }
    }
    ...
    T find(int l, int r) { // [l, r)
      ...
      return accum_func(sparse[cur_log][l],
                        sparse[cur_log][r - (1 << cur_log)]);
    }
  };
  endsnippet

      snippet decart "treap" struct Node {
    int x;
    ll y;
    int sz;
    Node *left;
    Node *right;
    Node(int x = 0) : x(x), y((ll)rnd()), sz(1), left(NULL), right(NULL) {}
  };
  ...
  pair<Node *, Node *> keySplit(Node *v,
                                int key) { // l's keys <= key, r's keys > key
    if (v == NULL) {
      return {v, v};
    }
    if (v->x <= key) {
  ...
  endsnippet
      ...
      snippet fenwick "Fenwick tree" struct Fenwick {
    vector<ll> tree;
    int n;

    Fenwick(int n) : n(n) { tree.assign(n, 0); }
  ...
  endsnippet
      ...
      snippet Fenwick2D "2D Fenwick tree" struct Fenwick2D {
    vector<vector<ll>> tree;
    int n, m;

    Fenwick2D(int n, int m) : n(n), m(m) { tree.assign(n, vector<ll>(m, 0)); }
  ...
  endsnippet
      ...
      snippet modular "modular arithmetics" template <int MODULO>
      struct ModularInt {
    int value;
  ...
  endsnippet
      ...
      snippet table "table graph" int dx[] = {0, 1, 0, -1};
  int dy[] = {1, 0, -1, 0};
  ...
  endsnippet
    ...
    "block" {$0} endsnippet
        ...
        snippet dsu "Disjoint Set Union" struct DSU {
      vector<int> pr;
      int n;
    ...
    template <typename F, typename S> void debug_print(pair<F, S> val) {
      cerr << "(";
    ...
    template <typename T> void debug_print(T val) {
      cerr << "{";
      bool first = true;
...
#define debug(...)                                                             \
  cerr << "@@@ " << #__VA_ARGS__ << " =";                                      \
  debug_print_collection(__VA_ARGS__);
